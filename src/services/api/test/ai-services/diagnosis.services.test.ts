...
/**
 *  Run it solely :
 *  jest src/services/api/test/ai-services/diagnosis.services.test.ts
 */
...
import {
  ...
  createTestPet_4,
  deleteUser,
  enrollFreeSubscription,
  errorChecker,
  userLogin,
} from "../main-services/helper";
...
describe("diagnosis", () => {
  ...
  let blood_report_folder_path = "../testing_data/blood-report";
  ...
  // mocking diagnosis ai function
  async function diagnosis(pet1_id, pet1_subscriptionId) {
    ...
    const { data } = await services.api.ai.diagnosis.processDiagnosis({
      accessToken,
      csrfToken,
      animal: {
        type: "",
  ...
  describe("Check if Service Subscription Created", () => {
    it("task 1 -  should service subscription created when pet accounts are created", async () => {
      const { data: servicesSubscriptions } =
        await services.api.ai.diagnosis.viewAllServiceSubscriptionByUserId(
      ...
      servicesSubscriptions.forEach((sub) => {
        ...
        expect(sub.unlimitedUse).toBe(false);
      });
  ...
  describe("serverHealthCheck()", () => {
    it("test 2 - should BVA-02 is running", async () => {
      const response = await services.api.ai.diagnosis.serverHealthCheck({
        accessToken,
        csrfToken,
        key: "testing",
      });
  ...
  describe("updateServiceSubscription()", () => {
    ...
    it("test 3 - update all five service subscriptions", async () => {
      ...
      await services.api.ai.diagnosis.updateServiceSubscription({
        petId: pet1_id,
        serviceSubscriptionId: pet1_subscriptionId,
        usageLimit: 4,
        unlimitedUse: false,
  ...
  describe("Status change with pet account status", () => {
    it("test 4 - should service subscription be inactivate if pet account is inactivate", async () => {
      ...
      // find the  subscripions 1
      const sub2 = servicesSubscriptions2.find(
        (sub) => sub.id === pet1_subscriptionId
      );

      expect(sub2?.status).toBe("INACTIVE");
      expect(sub2?.eventRecords[1].type).toBe("INACTIVATE");
    });
  ...
  describe("Access control", () => {
    ...
    it("Task 6 - should accessing to scanBloodReport() be not allowed for ineligible subscriptions", async () => {
      ...
      try {
        await diagnosis(pet2_id, pet2_subscriptionId);
      ...
      try {
        await diagnosis(pet4_id, pet4_subscriptionId);
        fail("Expected UnprocessableEntityException");
      } catch (e) {
        const { data } = e.response;
  ...
  describe("blood-reports()", () => {
    ...
    it.skip("Task 3 - should return extracted blood-reports data", async () => {
      // Trigger blood report scan
      let data = await diagnosis(pet1_id, pet1_subscriptionId);
      // expect(data.images).toBeDefined();
      // expect(data.data).toBeDefined();

      data = await diagnosis(pet5_id, pet5_subscriptionId);
      // expect(data.images).toBeDefined();
      // expect(data.data).toBeDefined();
    ...
    describe("reset the usage", () => {
      // sub 1 - `limited`, `status=active`, `limit=4`, `usage=0`, `nextResetDate='one month ago` ,  <<< ** reset
      ...
      it("Task 3 - should reset the usage of service subscription that are `active`,`limited` and `nextResetDate` expired", async () => {
        await services.api.ai.diagnosis.resetAllServiceSubscriptionUsage({
          accessToken,
          csrfToken,
        });
