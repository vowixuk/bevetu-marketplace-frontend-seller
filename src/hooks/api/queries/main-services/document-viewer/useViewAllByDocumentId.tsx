...
export const useViewAllByDocumentId = (
  payload: DocumentViewer.IViewAllByDocumentIdPayload
) => {
  const { setCsrfToken, csrfToken } = useAuthContext() as IAuthContext;
  const { data, error, isError, refetch, isLoading } = useQuery({
    queryKey: [
      QUERY_KEYS.MAIN.DOCUMENT_VIEWER.VIEW_ALL_BY_DOCUMENT_ID(
        payload.documentId
      ),
    ],
    queryFn: async () => {
      try {
        ...
        const data = await services.api.main.documentViewer.viewAllByDocumentId(
          {
            ...payload,
