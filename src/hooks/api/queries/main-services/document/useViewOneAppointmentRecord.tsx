...
export const useViewOneAppointmentRecord = (
  payload: Document.IViewOneAppointmentRecordPayload
) => {
  const { setCsrfToken, csrfToken } = useAuthContext() as IAuthContext;
  const { data, error, isError, refetch, isLoading } = useQuery({
    queryKey: [
      QUERY_KEYS.MAIN.DOCUMENT.VIEW_ONE_APPOINTMENT_RECORD(payload.reportId),
    ],
    queryFn: async () => {
      try {
        if (!csrfToken) {
