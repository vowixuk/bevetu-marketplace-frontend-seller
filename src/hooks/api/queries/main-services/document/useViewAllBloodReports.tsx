...
export const useViewAllBloodReports = (
  payload: Document.IViewAllBloodReportsPayload,
  p1: { enabled: boolean }
) => {
  const { setCsrfToken, csrfToken } = useAuthContext() as IAuthContext;
  const { data, error, isError, refetch, isLoading } = useQuery({
    queryKey: [QUERY_KEYS.MAIN.DOCUMENT.VIEW_ALL_BLOOD_REPORTS(payload.petId)],
    queryFn: async () => {
      try {
        if (!csrfToken) {
          throw new Error("CSRF token is not available.");
        }
        const data = await services.api.main.document.viewAllBloodReports({
          ...payload,
