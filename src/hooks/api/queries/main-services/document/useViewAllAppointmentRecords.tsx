...
export const useViewAllAppointmentRecords = (
  payload: Document.IViewAllAppointmentRecordsPayload,
  p1: { enabled: boolean }
) => {
  const { setCsrfToken, csrfToken } = useAuthContext() as IAuthContext;
  const { data, error, isError, refetch, isLoading } = useQuery({
    queryKey: [QUERY_KEYS.MAIN.DOCUMENT.VIEW_ALL_APPOINTMENT_RECORDS(payload.petId)],
    queryFn: async () => {
      try {
        if (!csrfToken) {
          throw new Error("CSRF token is not available.");
        }
        const data = await services.api.main.document.viewAllAppointmentRecords({
          ...payload,
