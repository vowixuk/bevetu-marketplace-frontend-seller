...
export const useViewOneVaccineRecord = (
  payload: Document.IViewOneVaccineRecordPayload,
) => {
  const { setCsrfToken, csrfToken } = useAuthContext() as IAuthContext;
  const { data, error, isError, refetch, isLoading } = useQuery({
    queryKey: [
      QUERY_KEYS.MAIN.DOCUMENT.VIEW_ONE_VACCINE_RECORD(`${payload.reportId}-${payload.petId}`),
    ],
    queryFn: async () => {
      try {
        if (!csrfToken) {
