...
export const useViewAllVaccineRecords = (
  payload: Document.IViewAllVaccineRecordsPayload,
  p1: { enabled: boolean }
) => {
  const { setCsrfToken, csrfToken } = useAuthContext() as IAuthContext;
  const { data, error, isError, refetch, isLoading } = useQuery({
    queryKey: [QUERY_KEYS.MAIN.DOCUMENT.VIEW_ALL_VACCINE_RECORDS(payload.petId)],
    queryFn: async () => {
      try {
        if (!csrfToken) {
          throw new Error("CSRF token is not available.");
        }
        const data = await services.api.main.document.viewAllVaccineRecords({
          ...payload,
