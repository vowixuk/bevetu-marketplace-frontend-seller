...
export const useViewUserRecentSubscription = () => {
  const { setCsrfToken, csrfToken } = useAuthContext() as IAuthContext;
  const { data, error, isError, refetch, isLoading } = useQuery({
    queryKey: [QUERY_KEYS.MAIN.SUBSCRIPTION.VIEW_USER_RECENT_SUBSCRIPTION],
    queryFn: async () => {
      try {
        if (!csrfToken) {
          throw new Error("CSRF token is not available.");
        }
        const data = await services.api.main.subscription.viewUserRecentSubscription({
