...
export const useGetNextStepSuggestion = () => {
  const { setCsrfToken, csrfToken } = useAuthContext() as IAuthContext;

  const { mutate, isError, error, data, isPending } = useMutation({
    mutationFn: async (payload: Diagnosis.IGetNextStepSuggestionPayload) => {
      if (!csrfToken) {
        throw new Error("CSRF token is not available.");
      }

      return await services.api.ai.diagnosis.getNextStepSuggestion({
        ...payload,
        csrfToken,
      });
    },
    ...
    onError: (error) => {
      console.log(error, "<<< error useGetNextStepSuggestion");
    },
  });
  return {
    mutate,
