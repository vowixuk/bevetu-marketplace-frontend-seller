...
export const useResetAllServiceSubscriptionUsage = () => {
  ...
  const { mutate, isError, error, data, isPending } = useMutation({
    mutationFn: async (payload: DocumentScanner.IResetAllServiceSubscriptionUsagePayload) => {
      if (!csrfToken) {
        throw new Error("CSRF token is not available.");
      }

      return await services.api.ai.documentScanner.resetAllServiceSubscriptionUsage({
        ...payload,
        csrfToken,
      });
    },
    onSuccess: (data) => {
      ...
      queryClient.invalidateQueries([QUERY_KEYS.AI.DOCUMENT_SCANNER.VIEW_ALL_SERVICE_SUBSCRIPTION] as any);
    },
    onError: (error) => {
      console.log(error, "<<< error useResetAllServiceSubscriptionUsage");
    },
  });
  return {
    mutate,
