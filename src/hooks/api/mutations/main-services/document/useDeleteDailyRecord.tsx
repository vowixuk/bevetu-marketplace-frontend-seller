...
export const useDeleteDailyRecord = () => {
  ...
  const { mutate, isError, error, data, isPending } = useMutation({
    mutationFn: async (payload: Document.IDeleteDailyRecordPayload) => {
      if (!csrfToken) {
        throw new Error("CSRF token is not available.");
      }

      const response = await services.api.main.document.deleteDailyRecord({
        ...payload,
        csrfToken,
      });
      return response;
    ...
    onSuccess: (data) => {
      ...
      queryClient.removeQueries([
        QUERY_KEYS.MAIN.DOCUMENT.VIEW_ONE_DAILY_RECORD(documentId),
      ] as any);
    },
    onError: (error) => {},
  });
