...
export const useDeleteBloodReport = () => {
  ...
  const { mutate, isError, error, data, isPending } = useMutation({
    mutationFn: async (payload: Document.IDeleteBloodReportPayload) => {
      if (!csrfToken) {
        throw new Error("CSRF token is not available.");
      }

      const response = await services.api.main.document.deleteBloodReport({
        ...payload,
        csrfToken,
      });
      return response;
    ...
    onSuccess: (data) => {
      ...
      queryClient.removeQueries([
        QUERY_KEYS.MAIN.DOCUMENT.VIEW_ONE_BLOOD_REPORT(documentId),
      ] as any);
    },
    onError: (error) => {},
  });
