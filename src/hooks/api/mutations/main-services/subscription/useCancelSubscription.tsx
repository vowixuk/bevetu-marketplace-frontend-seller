...
export const useCancelSubscription = () => {
  ...
  const { mutate, isError, error, data, isPending } = useMutation({
    mutationFn: async (payload: Subscription.ICancelSubscriptionPayload) => {
      if (!csrfToken) {
        throw new Error("CSRF token is not available.");
      }

      const response = await services.api.main.subscription.cancelSubscription({
        ...payload,
        csrfToken,
      });
      return response;
    ...
    onSuccess: (data) => {
      setCsrfToken(data.headers["x-csrf-token"]);
        queryClient.invalidateQueries([
          QUERY_KEYS.MAIN.USER.VIEW_PROFILE,
        ] as any);
        queryClient.invalidateQueries([
