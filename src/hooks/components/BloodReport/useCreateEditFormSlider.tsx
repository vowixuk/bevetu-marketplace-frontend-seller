...
import { useViewOneBloodReport } from "hooks/api/queries";
import {
...
export const useCreateEditFormSlider = ({
  ...
  const { mutate: createRecord }   = useCreateBloodReport();
  const { mutate: updateRecord }   = useUpdateBloodReport();
  const { mutate: deleteDocument } = useDeleteDocumentFromStorage();

  // Set Record data (Related to Edit mode only)
  const { data: existingRecordData } = useViewOneBloodReport({
  ...
  const createRecordOnClickHandler = () => {
    ...
    const validation = schemaValidation(
      DocumentSchemas.createBloodReport,
      payload
    );
    ...
    createRecord(payload as DocumentType.ICreateBloodReportPayload, {
      onSuccess: (data) => {
  ...
  const updateRecordOnClickHandler = () => {
    ...
    updateRecord(payload as DocumentType.IUpdateBloodReportPayload, {
      onSuccess: (data) => {
        // setIsOpen(false);
        onClose();
      },
