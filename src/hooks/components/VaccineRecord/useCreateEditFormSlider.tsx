...
export const useCreateEditFormSlider = ({
  ...
  const { mutate: createRecord } = useCreateVaccineRecord();
  const { mutate: updateRecord } = useUpdateVaccineRecord();
  const { mutate: deleteDocument } = useDeleteDocumentFromStorage();

  // Set Daily Record data (Related to Edit mode only)
  const { data: existingRecordData } = useViewOneVaccineRecord({
  ...
  const createRecordOnClickHandler = () => {
    ...
    const validation = schemaValidation(
      DocumentSchemas.createVaccineRecord,
      payload
    );
    ...
    createRecord(payload as DocumentType.ICreateVaccineRecordPayload, {
      onSuccess: (data) => {
        onClose();
      },
      onError: (err) => {},
  ...
  const updateRecordOnClickHandler = () => {
    ...
    const validation = schemaValidation(
      DocumentSchemas.updateVaccineRecord,
      payload
    );
    ...
    updateRecord(payload as DocumentType.IUpdateVaccineRecordPayload, {
      onSuccess: (data) => {
        onClose();
      },
      onError: (err) => {},
